  ...
  if (loading) {
    return (
      <main style={{ background: "#000", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <Navbar />
        <p style={{ color: "rgba(255,255,255,0.4)", letterSpacing: "0.2em" }}>Loading...</p>
      </main>
    );
  }
        ...
        <div style={{ maxWidth: "80rem", margin: "0 auto", padding: isMobile ? "6rem 1rem 4rem" : "8rem 1.5rem 6rem" }}>
          ...
          <div className="detail-grid">
            ...
            <div>
              ...
              <h1 style={{ fontFamily: "Georgia, serif", fontSize: "clamp(2rem, 5vw, 3.5rem)", fontWeight: 900, color: "#fff", textTransform: "uppercase", lineHeight: 1, margin: "0.5rem 0" }}>
                {model.name}
              </h1>
              ...
              <div style={{ padding: "1.25rem", border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.02)", marginBottom: "1.25rem" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "0.5rem" }}>
                  <div>
                    ...
                    <p style={{ color: "#fff", fontSize: "clamp(1.5rem, 4vw, 2rem)", fontWeight: 800 }}>${model.price?.toLocaleString()}</p>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.65rem", letterSpacing: "0.1em" }}>Est. monthly</p>
                    <p style={{ color: model.color || "#E31937", fontSize: "clamp(0.9rem, 2.5vw, 1.1rem)", fontWeight: 700 }}>
                      ${Math.round((model.price || 0) / 60).toLocaleString()}/mo
                    </p>
                  </div>
                </div>
              </div>
          ...
          {model.specs && Object.keys(model.specs).length > 0 && (
            <div style={{ marginTop: "5rem", borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "3.5rem" }}>
              <p style={{ color: model.color || "#E31937", fontSize: "0.7rem", letterSpacing: "0.4em", textTransform: "uppercase", marginBottom: "0.75rem" }}>Specifications</p>
              <h2 style={{ fontFamily: "Georgia, serif", fontSize: "clamp(1.75rem, 4vw, 3rem)", fontWeight: 900, color: "#fff", textTransform: "uppercase", marginBottom: "2.5rem" }}>Full Specs</h2>
          ...
          {model.features?.length > 0 && (
            <div style={{ marginTop: "4rem" }}>
              <p style={{ color: model.color || "#E31937", fontSize: "0.7rem", letterSpacing: "0.4em", textTransform: "uppercase", marginBottom: "0.75rem" }}>Included</p>
              <h2 style={{ fontFamily: "Georgia, serif", fontSize: "clamp(1.75rem, 4vw, 3rem)", fontWeight: 900, color: "#fff", textTransform: "uppercase", marginBottom: "2.5rem" }}>Key Features</h2>
              <div className="features-grid">
                {model.features.map((f, i) => (
                  <div key={i} style={{ padding: "1.25rem", border: "1px solid rgba(255,255,255,0.08)", display: "flex", alignItems: "center", gap: "1rem", background: "rgba(255,255,255,0.02)" }}>
                    <div style={{ width: 6, height: 6, borderRadius: "50%", background: model.color || "#E31937", flexShrink: 0 }} />
