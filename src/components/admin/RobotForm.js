...
const CATEGORIES = ["home", "professional", "industrial", "research"];

export default function RobotForm({ robot, onSave, onCancel }) {
  const isEditing = !!robot;

  const [form, setForm] = useState({
    name: robot?.name || "",
    category: robot?.category || "home",
    price: robot?.price || "",
    description: robot?.description || "",
    image: robot?.image || "",
    specs: robot?.specs?.join("\n") || "",
    badge: robot?.badge || "",
    inStock: robot?.inStock !== false,
  });
    ...
    try {
      ...
      const res = isEditing
        ? await fetch("/api/robots", {
            ...
            body: JSON.stringify({ id: robot._id, ...payload }),
          })
        : await fetch("/api/robots", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.08)", padding: "clamp(1.25rem, 4vw, 2rem)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem", flexWrap: "wrap", gap: "1rem" }}>
        <h2 style={{ fontFamily: "Georgia, serif", fontSize: "1.25rem", fontWeight: 700, color: "#fff", textTransform: "uppercase" }}>
          {isEditing ? `Edit ${robot.name}` : "Add New Robot"}
        </h2>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "1rem", marginBottom: "1.5rem" }}>
          <div>
            ...
            <input name="name" value={form.name} onChange={handleChange} required placeholder="Optimus Gen 2 Home" style={inputStyle} onFocus={(e) => (e.target.style.borderColor = "#E31937")} onBlur={(e) => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
          </div>
          <div>
            <label style={labelStyle}>Price (USD) *</label>
            <input name="price" type="number" value={form.price} onChange={handleChange} required placeholder="19999" style={inputStyle} onFocus={(e) => (e.target.style.borderColor = "#E31937")} onBlur={(e) => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
          </div>
          <div>
          ...
          <div>
            ...
            <input name="badge" value={form.badge} onChange={handleChange} placeholder="New, Popular, Limited" style={inputStyle} onFocus={(e) => (e.target.style.borderColor = "#E31937")} onBlur={(e) => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
          </div>
          <div style={{ gridColumn: "1 / -1" }}>
            <label style={labelStyle}>Description</label>
            <textarea name="description" value={form.description} onChange={handleChange} rows={3} placeholder="Describe this robot..." style={{ ...inputStyle, resize: "vertical", fontFamily: "inherit", lineHeight: 1.6 }} onFocus={(e) => (e.target.style.borderColor = "#E31937")} onBlur={(e) => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
          </div>
        </div>
        ...
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1rem" }}>
            <input name="image" value={form.image} onChange={handleChange} placeholder="Or paste an image URL" style={{ ...inputStyle, flex: 1 }} onFocus={(e) => (e.target.style.borderColor = "#E31937")} onBlur={(e) => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
            {form.image && (
              <button type="button" onClick={() => setForm((prev) => ({ ...prev, image: "" }))} style={{ padding: "0.75rem 1rem", background: "rgba(227,25,55,0.1)", border: "1px solid rgba(227,25,55,0.3)", color: "#E31937", fontSize: "0.75rem", cursor: "pointer", whiteSpace: "nowrap" }}>
                Clear
              </button>
            )}
          </div>
        ...
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <textarea name="specs" value={form.specs} onChange={handleChange} rows={4} placeholder={"5'8\" height\n~57 lbs\n8hr battery\nWi-Fi & BT 5.0"} style={{ ...inputStyle, resize: "vertical", fontFamily: "inherit", lineHeight: 1.6 }} onFocus={(e) => (e.target.style.borderColor = "#E31937")} onBlur={(e) => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
        </div>
        ...
        <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
          <button type="submit" disabled={saving || uploading} style={{ padding: "0.875rem 2rem", background: saving ? "rgba(227,25,55,0.5)" : "#E31937", color: "#fff", border: "none", fontSize: "0.8rem", fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", cursor: saving ? "not-allowed" : "pointer" }}>
            {saving ? "Saving..." : isEditing ? "Update Robot" : "Add Robot"}
          </button>
